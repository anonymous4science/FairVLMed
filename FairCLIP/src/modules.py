...
class fair_vl_med_dataset(torch.utils.data.Dataset):
    def __init__(self, dataset_dir='', preprocess=None, files=None, subset='train', text_source='note', summarized_note_file=None, ruleout_unknown=False):
        ...
        if self.subset == 'train' and self.text_source == 'note' and summarized_note_file != '':
        ...
        if subset != 'train' or self.ruleout_unknown:
            tmp_files = []
    ...
    def __getitem__(self, idx):
        ...
        data = np.load(npz_path)
        slo_fundus = data['fundus_slo'].astype(np.float32) # original size: (664, 512)
...
class fair_vl_group_dataset(torch.utils.data.Dataset):
    def __init__(self, dataset_dir='', preprocess=None, files=None, subset='train', text_source='note', summarized_note_file=None, attribute='race', thegroup=0):
        self.preprocess = preprocess
        self.dataset_dir = os.path.join(dataset_dir, 'data')
        self.subset = subset
        self.text_source = text_source
...
class image_title_dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        npz_path = os.path.join(self.dataset_dir, self.files[idx])
        data = np.load(npz_path)
        slo_fundus = data['fundus_slo'].astype(np.float32)
        slo_fundus = self.preprocess(Image.fromarray(slo_fundus))
